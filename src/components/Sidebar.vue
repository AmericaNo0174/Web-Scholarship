  ...
  <div class="container-side">
    ...
    >
      ...
      <div class="img-box" v-else>
        <div class="">
          <img class="img-profile" :src="form.user_img" />
          
        </div>
        <div class="user-info">
          <p>Admin: {{ form.fname + " " + form.lname }}</p>
          <p>Kasetsart University</p>
        </div>
      </div>
      <router-link class="menu-list py-9" to="/open_capital"
        ><span
          ><i class="fas fa-graduation-cap"></i>ทุนที่เปิดให้ลงทะเบียน</span
        ></router-link
      >
       <router-link class="menu-list py-9" to="/step"
        ><span
          ><i class="fas fa-book"></i>ขั้นตอนการสมัครขอทุน</span
        ></router-link
      >
      ...
      <router-link class="menu-list py-9" to="/history"
        ><span
          ...
          >ประวัติรายชื่อผู้ที่ได้รับทุน</span
        ></router-link
      >
       <router-link class="menu-list py-9" to="/Role"
        ><span
          ><i class="fas fa-user-tag"></i>แก้ไข Role</span
        ></router-link
      >
    </div>
    <div
      v-if="$store.state.user.Role == 2 || $store.state.user.Role ==null"
      class="sidebar-box"
      :class="{ 'w-0': !sidebarOpen, 'w-200': sidebarOpen }"
    >
      <div class="img-box" v-if="!isShow">
      ...
      <div class="img-box" v-else>
        ...
        <div class="user-info">
          <p> User: {{ form.fname + " " + form.lname }}</p>
          <p>Kasetsart University</p>
        </div>
      ...
      <router-link class="menu-list py-9" to="/profile"
        ><span
          ><i class="fas fa-user-circle"></i>ข้อมูลส่วนตัว</span
        ></router-link
      >
      <router-link class="menu-list py-9" to="/open_capital"
        ><span
    ...
    >
      ...
      <div class="img-box" v-else>
        ...
        <div class="user-info">
          <p>คณะกรรมการ: {{ form.fname + " " + form.lname }}</p>
          <p>Kasetsart University</p>
        </div>
      </div>
      <router-link class="menu-list py-9" to="/open_capital"
